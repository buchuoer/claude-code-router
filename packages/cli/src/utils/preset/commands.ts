...
async function listPresets(): Promise<void> {
  ...
  try {
    ...
    console.log(`\nCreate your first preset with: ${GREEN}myccr preset export <name>${RESET}\n`);
    return;
  }
  ...
  if (presetDirs.length === 0) {
    console.log('\nNo presets found.');
    console.log(`\nInstall a preset with: ${GREEN}myccr preset install <file>${RESET}\n`);
    return;
  }
  ...
  switch (subCommand) {
    case 'export':
      ...
      if (!presetName) {
        ...
        console.error('Usage: myccr preset export <name> [--output <path>] [--description <text>] [--author <name>] [--tags <tags>]\n');
        process.exit(1);
      }
    ...
    case 'install':
      ...
      if (!source) {
        ...
        console.error('Usage: myccr preset install <file | url | name>\n');
        process.exit(1);
      }
    ...
    case 'remove':
      ...
      if (!deleteName) {
        console.error('\nError: Preset name is required\n');
        console.error('Usage: myccr preset delete <name>\n');
        process.exit(1);
      }
    ...
    case 'info':
      ...
      if (!infoName) {
        console.error('\nError: Preset name is required\n');
        console.error('Usage: myccr preset info <name>\n');
        process.exit(1);
      }
    ...
    default:
      ...
      console.error('  myccr preset export <name>      Export current configuration as a preset');
      console.error('  myccr preset install <source>   Install a preset from file, URL, or registry');
      console.error('  myccr preset list              List installed presets');
      console.error('  myccr preset info <name>        Show preset information');
      console.error('  myccr preset delete <name>      Delete a preset\n');
      process.exit(1);
  }
}
